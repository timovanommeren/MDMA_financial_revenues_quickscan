...
ggplot(data.frame(revenue), aes(x = "", y = revenue)) +
  geom_boxplot(aes(fill = "Estimated \nvariation in \nannual revenue \n(1st–90th percentile)"),
               width = 0.5, outlier.shape = NA) +
  coord_cartesian(ylim = quantile(revenue, c(0.01, 0.90))) +
  labs(
    title = "Projected Annual Revenue of a State MDMA Monopoly",
    x = NULL, y = "Revenue"
  ) +
  scale_y_continuous(labels = label_number(prefix = "€")) +
  geom_hline(aes(yintercept = revenue_estimate, color = "Estimated \nlong-term \nannual revenue"),
             linewidth = 1) +
  scale_fill_manual(values = c("Estimated \nvariation in \nannual revenue \n(1st–90th percentile)" = "grey90"), name = NULL) +
  scale_color_manual(values = c("Estimated \nlong-term \nannual revenue" = "blue"), name = NULL) +
  guides(
...
ggplot(data.frame(revenue), aes(x = "", y = revenue)) +
  geom_boxplot(aes(fill = "Estimated variation in annual revenue (1st–90th percentile)"),
               width = 0.5, outlier.shape = NA) +
  coord_cartesian(ylim = quantile(revenue, c(0.01, 0.90))) +
  labs(
    title = "Projected Annual Revenue of a State MDMA Monopoly",
  ...
  geom_segment(data = data.frame(y = revenue_estimate),
             ...
             linewidth = 1) +
  scale_fill_manual(
    values = c("Estimated variation in annual revenue (1st–90th percentile)" = "grey90"),
    labels = c(
      "Estimated variation in annual revenue (1st–90th percentile)" =
        "Estimated variation in annual revenue <br><span style='color:grey50'>(1st–90th percentile)</span>"
    ),
    name = NULL
  ) +
